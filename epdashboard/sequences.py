...
class PromptCache:
    ...
    def add(self, gid: int, text: str, X: np.ndarray, positions: np.ndarray):
        ...
        sims = (Xc / norms[:, None]) @ d.E.T          # (T, K)
        row_of = {int(p): r for r, p in enumerate(positions)}
        ids = self.tok.encode(text, add_special_tokens=False)
        self._by_gid[gid] = {"ids": ids, "sims": sims, "norms": norms,
                             "best": np.argmax(sims, axis=1), "row_of": row_of}
    ...
    def sequence(self, gid: int, pos: int, region: int,
        ...
        if entry is None:
            return None
        d = self.d
        ids, sims, norms = entry["ids"], entry["sims"], entry["norms"]
        ...
        for t in range(lo, hi):
            ...
            if row is None:                          # position 0 (sink) — no act
                acts.append(None)
                continue
            acts.append(round(float(sims[row, region] * norms[row]), 3))
            if (entry["best"][row] == region
                    and 1.0 - sims[row, region] <= d.threshold):
                mb.append(t - lo)
