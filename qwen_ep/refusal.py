...
Staged + resumable — artifacts land under runs/behavioral/<model>/:
...
Run:
    python -m qwen_ep.refusal --stage sanity
    python -m qwen_ep.refusal --stage probe --probe-layers 14,20,27,31
    python -m qwen_ep.refusal --stage build --percentile 12 --seed 0
    python -m qwen_ep.refusal --stage label
    python -m qwen_ep.refusal --stage ablate --percentile 12 --seed 0
    python -m qwen_ep.refusal --stage report
"""
...
from .adapter import (QwenModel, make_extract_fn, make_final_extract_fn,
                      model_tag)

logger = logging.getLogger("qwen_ep.refusal")
...
def main() -> None:
    ...
    root = Path("runs/behavioral") / model_tag(args.model_id)
    root.mkdir(parents=True, exist_ok=True)
